...
async def ping(e):
    if e.sender_id in SUDO_USERS:
        start = datetime.now()
        jarvis = await e.reply(f"ᴥ︎︎︎ ꪀꪖ𝘬𝘴ꫝ ✔︎")
        end = datetime.now()
        mp = (end - start).microseconds / 1000
        await jarvis.edit(f"[ꪀꪖ𝘬𝘴ꫝ 𝓲𝘴 𝘳ꫀꪖᦔꪗ 𝓽ꪮ  ](https://t.me/naksh_accounts)[𝐅𝐔𝐂𝐊 𝐇𝐀𝐓𝐄𝐑𝐒 🥀](https://t.me/naksh_accounts)🤖\n» `{mp} ᴍꜱ`")
...
async def addsudo(event):
    if event.sender_id == OWNER_ID:
        ...
        ok = await event.reply(f"» __naksh Ka Ek Beta modulars ko chodhta hai..__")
        target = ""
        ...
        if str(target) in sudousers:
            await ok.edit(f"YE BHI NAKSH KA HI BACHA HAI.. !!")
        else:
            if len(sudousers) > 0:
...
async def removesudo(event):
    if event.sender_id == OWNER_ID:
        Heroku = heroku3.from_key(HEROKU_API_KEY)
        sudousers = getenv("SUDO_USERS", default=None)
        ok = await event.reply(f"YE Naksh Ki Najayaz Aulad thi isiliye nikal diya💋...")
        target = ""
        if HEROKU_APP_NAME is not None:
            app = Heroku.app(HEROKU_APP_NAME)
        else:
        ...
        await event.reply("Only Naksh can remove sudo users.")
...
async def show_sudo_users(event):
    if event.sender_id == OWNER_ID:
        sudo_users_list = "Naksh Ke Bache hai ye:\n"
        for user_id in SUDO_USERS:
            sudo_users_list += f"- {user_id}\n"
        await event.reply(sudo_users_list)
    else:
        await event.reply("Only Naksh view the sudo users list.")
...
async def addmultisudo(event):
    if event.sender_id == OWNER_ID:
        ...
        await event.reply("Only Naksh can add sudo users.")
